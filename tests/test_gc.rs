...
fn zero_refcount_is_reconciled_before_freeing_live_pba() {
    use onyx_storage::packer::packer::new_hole_map;
    use onyx_storage::zone::worker::ZoneWorker;

    let env = setup_gc_env();
    let hole_map = new_hole_map();

    let vol_a = "reconcile-a";
    let vol_b = "reconcile-b";
    register_volume(&env.meta, vol_a, CompressionAlgo::Lz4, 100);
    register_volume(&env.meta, vol_b, CompressionAlgo::Lz4, 200);
    ...
    let data_a = vec![0x41; BLOCK_SIZE as usize];
    let data_b = vec![0x52; BLOCK_SIZE as usize];
    let data_a_new = vec![0x63; BLOCK_SIZE as usize];
    ...
    let mapping_a = env
        .meta
        .get_mapping(&VolumeId(vol_a.to_string()), Lba(0))
        .unwrap()
        .unwrap();
    let mapping_b = env
        .meta
        .get_mapping(&VolumeId(vol_b.to_string()), Lba(0))
        .unwrap()
        .unwrap();
    assert_eq!(
        mapping_a.pba, mapping_b.pba,
        "fixture must use a shared packed slot"
    );
    let shared_pba = mapping_a.pba;
    assert_eq!(env.meta.get_refcount(shared_pba).unwrap(), 2);

    // Simulate the on-disk drift seen in soak: blockmap still has two live refs,
    // but refcount was under-counted to 1.
    env.meta.set_refcount(shared_pba, 1).unwrap();

    env.pool.append(vol_a, Lba(0), 1, &data_a_new, 100).unwrap();
    assert!(wait_for_flush(&env.pool, 5000), "overwrite flush timeout");

    let repaired = env.meta.get_refcount(shared_pba).unwrap();
    assert_eq!(
        repaired, 1,
        "cleanup must reconcile the surviving live ref instead of freeing the shared PBA"
    );
    ...
    assert_eq!(
        worker.handle_read(vol_a, Lba(0)).unwrap().unwrap(),
        data_a_new
    );
    assert_eq!(
        worker.handle_read(vol_b, Lba(0)).unwrap().unwrap(),
        data_b,
        "surviving fragment must remain readable after refcount repair"
    );
